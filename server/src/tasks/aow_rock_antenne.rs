    ...
    match songs.last() {
        Some(&last) => {
            match pattern.captures(last) {
                ...
                None => Err(TasksError::Internal(format!("Couldn't match regex for track count on AOW!")))
            }
        }
        None => Err(TasksError::Internal(format!("Didn't find the last song of album!")))
    }
}
    ...
    match possible_artist {
        Some(artist_el) => {
            match artist_el.text().next() {
                ...
                None => Err(TasksError::Internal("No Text found in artist element selector!".to_string()))
            }
        }
        None => Err(TasksError::Internal("No Element found in artist element selector!".to_string()))
    }
}
...
fn select_album(overview: &scraper::Html) -> Result<String> {
    ...
    match possible_album {
        ...
        None => Err(TasksError::Internal("No Element found in album element selector!".to_string()))
    }
}
    ...
    match possible_date {
        Some(date_el) => {
            match date_el.value().attr("content") {
                ...
                None => Err(TasksError::Internal("No Attribute named content found in date element!".to_string()))
            }
        }
        None => Err(TasksError::Internal("No Element found in date element selector!".to_string()))
    }
}
    ...
    match possible_link {
        Some(link_el) => {
            match link_el.value().attr("href") {
                Some(c) => Ok(c.to_string()),
                None => Err(TasksError::Internal("No Attribute named href found in full post link element!".to_string()))
            }
        }
        None => Err(TasksError::Internal("No Element found in full post link element selector!".to_string()))
    }
}
...
fn select_reasoning_html(full_post: &scraper::Html) -> Result<String> {
    ...
    match possible_reasoning {
        ...
        None => Err(TasksError::Internal("No Element found in reasoning element selector!".to_string()))
    }
}
...
fn select_song_list(full_post: &scraper::Html) -> Result<String> {
    ...
    match possible_song_list {
        ...
        None => Err(TasksError::Internal("No Element found in song list element selector!".to_string()))
    }
}