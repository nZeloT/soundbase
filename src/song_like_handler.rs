...
pub fn consume_like_message<DB :SongDB>(db: &mut DB, buffer : Vec<u8>) -> error::Result<Vec<u8>> {
    ...
    let resp_kind = process_message(db, &msg.payload_as_request().unwrap())?;
...
fn process_message<DB :SongDB>(db: &mut DB, request: &song_like_protocol_generated::Request) -> error::Result<song_like_protocol_generated::ResponseKind> {

    println!("from requesting party {:?}", request.requesting_party());

    let mut song_info = song_like::Song::new(&request.song_info());
    let song_source = song_like::SongSource::new(&request.song_source_info());
